...
public class InterceptorExConfig implements WebMvcConfigurer { // WebMvcConfiguerer 인터페이스를 구현하여 Interceptor 설정 클래스로 사용한다.

	/*
	@Autowired
    private InterceptorEx interceptorEx;	// Interceptor로직을 사용할 객체생성
    ...
    public void addInterceptors(InterceptorRegistry registry) { // addInterceptors 메서드로 Interceptor 설정 지정
        registry.addInterceptor(interceptorEx)			 		// Interceptor로직을 사용할 객체를 지정
                ...
                .addPathPatterns("/admin/*" , "/management/menu1"); 					 	// Interceptor를 적용할 URL 패턴을 지정
